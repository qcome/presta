...
if [ -z "$1" ]; then
    ...
    docker-compose -f ./docker/docker-compose.yml --project-name presta-$app_name down
    APP_NAM=$app_name RUN_INSTALL=$run_install docker-compose -f ./docker/docker-compose.yml --project-name presta-$app_name up --build
fi