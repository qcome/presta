...
if [ -z "$1" ]; then
    ...
    docker-compose -f docker-compose-test-presta-image.yml down
    APP_NAME=$app_name RUN_INSTALL=$run_install docker-compose -f docker-compose-test-presta-image.yml up --build
fi